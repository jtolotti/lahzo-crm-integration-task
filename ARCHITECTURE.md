...
**Single Legacy App with OAuth:** HubSpot requires a Legacy App for webhook subscriptions (Private Apps don't support them). Rather than using a separate Private App for API access (which would mean two apps — something you'd never see in production), we use the same Legacy App for everything: **webhook delivery + signature verification** (via client secret) and **CRM API access** (via OAuth 2.0 tokens). The server implements a standard OAuth flow (`/hubspot/auth` → authorization redirect → `/hubspot/auth/callback` → code exchange) with automatic token refresh. Tokens are persisted to a local file (`.hubspot-tokens.json`, gitignored) and refreshed **proactively** — the token manager checks the expiry clock on every API call and refreshes 5 minutes before actual expiry, so the adapter never sees expired credentials and zero requests fail from token expiry. If a refresh fails transiently, the error propagates to BullMQ which retries with exponential backoff. In production, tokens would be stored in a secrets manager (AWS Secrets Manager, HashiCorp Vault) or encrypted in the database rather than a local file — the token manager's `loadFromDisk`/`saveToDisk` functions are the only swap point needed for that migration. This mirrors production-grade OAuth integration patterns where a single app handles both inbound events and outbound API calls.
